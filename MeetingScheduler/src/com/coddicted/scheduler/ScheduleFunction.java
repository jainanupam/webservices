...
public class ScheduleFunction {
    ...
    public static Boolean updateTime(String date) throws IOException {
    	...
    	List<String> schedule = ScheduleFunction.load("database.txt");
    	while(schedule.size() <= i) {
    		schedule.add("0");
    	}
    	if(schedule.get(i).equals("0")) {
    		schedule.set(i,"1");
    		save("database.txt",schedule);
    		return true;
    	}
    	return false;
    }
    
    public static Boolean isTimeAvailable(String date) throws IOException {
    	Integer i = Integer.parseInt(date);
    	List<String> schedule = ScheduleFunction.load("database.txt");
    	if(schedule.size() <= i){
    		return true;
